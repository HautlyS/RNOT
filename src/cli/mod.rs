...
pub async fn run(mut config: Config) -> Result<()> {
    ...
    match cli.command {
        Commands::Tui => {
            ...
            if token.is_none() {
                println!("Warning: No Telegram token set. Use 'rnot set-token <TOKEN>' to set one.");
            }
            ...
            let telegram = TelegramClient::new(
                token,
                config.app_config.telegram_chat_id.clone(),
            );
            ...
            let monitor_handle = tokio::spawn(async move {
                ...
                monitor.run(sites_rx, events_tx, shutdown_tx_clone.subscribe()).await;
            });
        ...
        Commands::Add { url, name, selector } => {
        ...
        Commands::TelegramSetup => {
            let token = config.get_telegram_token()
                .ok_or_else(|| anyhow::anyhow!("No Telegram token set. Use 'rnot set-token <TOKEN>' first."))?;
            ...
            println!("Timeout: No messages received. Please send a message to your bot and try again.");
        }
        Commands::Daemon => {
            let token = config.get_telegram_token();
        ...
        Commands::Check => {
            ...
            for mut site in config.app_config.sites.clone() {
                match monitor.check_site(&mut site).await {
                    Ok(Some(diff)) => {
                        println!("Changed: {} - {}", site.name, diff.chars().take(100).collect::<String>());
        ...
        Commands::Status => {
            ...
            println!("Telegram Token: {}", if has_token { "✓ Set (encrypted)" } else { "✗ Not set" });
            
            let chat_id = config.app_config.telegram_chat_id.as_ref()
                .map(|_| "✓ Set")
                .unwrap_or("✗ Not set");
            println!("Telegram Chat ID: {}", chat_id);
            
            println!("Check Interval: {} seconds", config.app_config.check_interval_secs);
